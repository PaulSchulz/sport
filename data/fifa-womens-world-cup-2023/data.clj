{:event "fifa-womens-world-cup-2023",
 :data "data/fifa-womens-world-cup-2023/data.clj",
 :details
 {:date "",
  :custom "data/fifa-womens-world-cup-2023/custom.clj",
  :title "",
  :fixtures "data/fifa-womens-world-cup-2023/fixtures.json",
  :datadir "data/fifa-womens-world-cup-2023/",
  :url "",
  :code nil,
  :location "",
  :data "data/fifa-womens-world-cup-2023/data.clj"},
 :fixtures
 [{:RoundNumber 1,
   :Group "Group A",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "New Zealand",
   :DateUtc "2023-07-20 07:00:00Z",
   :AwayTeam "Norway",
   :Location "Eden Park",
   :MatchNumber 1}
  {:RoundNumber 1,
   :Group "Group B",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Australia",
   :DateUtc "2023-07-20 10:00:00Z",
   :AwayTeam "Republic of Ireland",
   :Location "Sydney Football Stadium",
   :MatchNumber 2}
  {:RoundNumber 1,
   :Group "Group B",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Nigeria",
   :DateUtc "2023-07-21 02:30:00Z",
   :AwayTeam "Canada",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 4}
  {:RoundNumber 1,
   :Group "Group A",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Philippines",
   :DateUtc "2023-07-21 05:00:00Z",
   :AwayTeam "Switzerland",
   :Location "Dunedin Stadium",
   :MatchNumber 3}
  {:RoundNumber 1,
   :Group "Group C",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Spain",
   :DateUtc "2023-07-21 07:30:00Z",
   :AwayTeam "Costa Rica",
   :Location "Wellington Regional Stadium",
   :MatchNumber 5}
  {:RoundNumber 1,
   :Group "Group E",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "USA",
   :DateUtc "2023-07-22 01:00:00Z",
   :AwayTeam "Vietnam",
   :Location "Eden Park",
   :MatchNumber 9}
  {:RoundNumber 1,
   :Group "Group C",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Zambia",
   :DateUtc "2023-07-22 07:00:00Z",
   :AwayTeam "Japan",
   :Location "Waikato Stadium",
   :MatchNumber 6}
  {:RoundNumber 1,
   :Group "Group D",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "England",
   :DateUtc "2023-07-22 09:30:00Z",
   :AwayTeam "Group B Play-Off Winner",
   :Location "Brisbane Stadium",
   :MatchNumber 7}
  {:RoundNumber 1,
   :Group "Group D",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Denmark",
   :DateUtc "2023-07-22 12:00:00Z",
   :AwayTeam "China PR",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 8}
  {:RoundNumber 1,
   :Group "Group G",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Sweden",
   :DateUtc "2023-07-23 05:00:00Z",
   :AwayTeam "South Africa",
   :Location "Wellington Regional Stadium",
   :MatchNumber 12}
  {:RoundNumber 1,
   :Group "Group E",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Netherlands",
   :DateUtc "2023-07-23 07:30:00Z",
   :AwayTeam "Group A Play-Off Winner",
   :Location "Dunedin Stadium",
   :MatchNumber 10}
  {:RoundNumber 1,
   :Group "Group F",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "France",
   :DateUtc "2023-07-23 10:00:00Z",
   :AwayTeam "Jamaica",
   :Location "Sydney Football Stadium",
   :MatchNumber 11}
  {:RoundNumber 1,
   :Group "Group G",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Italy",
   :DateUtc "2023-07-24 06:00:00Z",
   :AwayTeam "Argentina",
   :Location "Eden Park",
   :MatchNumber 14}
  {:RoundNumber 1,
   :Group "Group H",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Germany",
   :DateUtc "2023-07-24 08:30:00Z",
   :AwayTeam "Morocco",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 15}
  {:RoundNumber 1,
   :Group "Group F",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Brazil",
   :DateUtc "2023-07-24 11:00:00Z",
   :AwayTeam "Group C Play-Off Winner",
   :Location "Hindmarsh Stadium",
   :MatchNumber 13}
  {:RoundNumber 1,
   :Group "Group H",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Colombia",
   :DateUtc "2023-07-25 02:00:00Z",
   :AwayTeam "Korea Republic",
   :Location "Sydney Football Stadium",
   :MatchNumber 16}
  {:RoundNumber 2,
   :Group "Group A",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "New Zealand",
   :DateUtc "2023-07-25 05:30:00Z",
   :AwayTeam "Philippines",
   :Location "Wellington Regional Stadium",
   :MatchNumber 17}
  {:RoundNumber 2,
   :Group "Group A",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Switzerland",
   :DateUtc "2023-07-25 08:00:00Z",
   :AwayTeam "Norway",
   :Location "Waikato Stadium",
   :MatchNumber 18}
  {:RoundNumber 2,
   :Group "Group C",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Japan",
   :DateUtc "2023-07-26 05:00:00Z",
   :AwayTeam "Costa Rica",
   :Location "Dunedin Stadium",
   :MatchNumber 21}
  {:RoundNumber 2,
   :Group "Group C",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Spain",
   :DateUtc "2023-07-26 07:30:00Z",
   :AwayTeam "Zambia",
   :Location "Eden Park",
   :MatchNumber 20}
  {:RoundNumber 2,
   :Group "Group B",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Canada",
   :DateUtc "2023-07-26 12:00:00Z",
   :AwayTeam "Republic of Ireland",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 19}
  {:RoundNumber 2,
   :Group "Group E",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "USA",
   :DateUtc "2023-07-27 01:00:00Z",
   :AwayTeam "Netherlands",
   :Location "Wellington Regional Stadium",
   :MatchNumber 23}
  {:RoundNumber 2,
   :Group "Group E",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Group A Play-Off Winner",
   :DateUtc "2023-07-27 07:30:00Z",
   :AwayTeam "Vietnam",
   :Location "Waikato Stadium",
   :MatchNumber 24}
  {:RoundNumber 2,
   :Group "Group B",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Australia",
   :DateUtc "2023-07-27 10:00:00Z",
   :AwayTeam "Nigeria",
   :Location "Brisbane Stadium",
   :MatchNumber 22}
  {:RoundNumber 2,
   :Group "Group G",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Argentina",
   :DateUtc "2023-07-28 00:00:00Z",
   :AwayTeam "South Africa",
   :Location "Dunedin Stadium",
   :MatchNumber 27}
  {:RoundNumber 2,
   :Group "Group D",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "England",
   :DateUtc "2023-07-28 08:30:00Z",
   :AwayTeam "Denmark",
   :Location "Sydney Football Stadium",
   :MatchNumber 25}
  {:RoundNumber 2,
   :Group "Group D",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "China PR",
   :DateUtc "2023-07-28 11:00:00Z",
   :AwayTeam "Group B Play-Off Winner",
   :Location "Hindmarsh Stadium",
   :MatchNumber 26}
  {:RoundNumber 2,
   :Group "Group G",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Sweden",
   :DateUtc "2023-07-29 07:30:00Z",
   :AwayTeam "Italy",
   :Location "Wellington Regional Stadium",
   :MatchNumber 30}
  {:RoundNumber 2,
   :Group "Group F",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "France",
   :DateUtc "2023-07-29 10:00:00Z",
   :AwayTeam "Brazil",
   :Location "Brisbane Stadium",
   :MatchNumber 28}
  {:RoundNumber 2,
   :Group "Group F",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Group C Play-Off Winner",
   :DateUtc "2023-07-29 12:30:00Z",
   :AwayTeam "Jamaica",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 29}
  {:RoundNumber 2,
   :Group "Group H",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Korea Republic",
   :DateUtc "2023-07-30 04:30:00Z",
   :AwayTeam "Morocco",
   :Location "Hindmarsh Stadium",
   :MatchNumber 32}
  {:RoundNumber 2,
   :Group "Group H",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Germany",
   :DateUtc "2023-07-30 09:30:00Z",
   :AwayTeam "Colombia",
   :Location "Sydney Football Stadium",
   :MatchNumber 31}
  {:RoundNumber 3,
   :Group "Group A",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Switzerland",
   :DateUtc "2023-07-30 07:00:00Z",
   :AwayTeam "New Zealand",
   :Location "Dunedin Stadium",
   :MatchNumber 33}
  {:RoundNumber 3,
   :Group "Group A",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Norway",
   :DateUtc "2023-07-30 07:00:00Z",
   :AwayTeam "Philippines",
   :Location "Eden Park",
   :MatchNumber 34}
  {:RoundNumber 3,
   :Group "Group C",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Japan",
   :DateUtc "2023-07-31 07:00:00Z",
   :AwayTeam "Spain",
   :Location "Wellington Regional Stadium",
   :MatchNumber 37}
  {:RoundNumber 3,
   :Group "Group C",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Costa Rica",
   :DateUtc "2023-07-31 07:00:00Z",
   :AwayTeam "Zambia",
   :Location "Waikato Stadium",
   :MatchNumber 38}
  {:RoundNumber 3,
   :Group "Group B",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Canada",
   :DateUtc "2023-07-31 10:00:00Z",
   :AwayTeam "Australia",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 35}
  {:RoundNumber 3,
   :Group "Group B",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Republic of Ireland",
   :DateUtc "2023-07-31 10:00:00Z",
   :AwayTeam "Nigeria",
   :Location "Brisbane Stadium",
   :MatchNumber 36}
  {:RoundNumber 3,
   :Group "Group E",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Group A Play-Off Winner",
   :DateUtc "2023-08-01 07:00:00Z",
   :AwayTeam "USA",
   :Location "Eden Park",
   :MatchNumber 41}
  {:RoundNumber 3,
   :Group "Group E",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Vietnam",
   :DateUtc "2023-08-01 07:00:00Z",
   :AwayTeam "Netherlands",
   :Location "Dunedin Stadium",
   :MatchNumber 42}
  {:RoundNumber 3,
   :Group "Group D",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "China PR",
   :DateUtc "2023-08-01 11:00:00Z",
   :AwayTeam "England",
   :Location "Hindmarsh Stadium",
   :MatchNumber 39}
  {:RoundNumber 3,
   :Group "Group D",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Group B Play-Off Winner",
   :DateUtc "2023-08-01 11:00:00Z",
   :AwayTeam "Denmark",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 40}
  {:RoundNumber 3,
   :Group "Group G",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Argentina",
   :DateUtc "2023-08-02 07:00:00Z",
   :AwayTeam "Sweden",
   :Location "Waikato Stadium",
   :MatchNumber 45}
  {:RoundNumber 3,
   :Group "Group G",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "South Africa",
   :DateUtc "2023-08-02 07:00:00Z",
   :AwayTeam "Italy",
   :Location "Wellington Regional Stadium",
   :MatchNumber 46}
  {:RoundNumber 3,
   :Group "Group F",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Group C Play-Off Winner",
   :DateUtc "2023-08-02 10:00:00Z",
   :AwayTeam "France",
   :Location "Sydney Football Stadium",
   :MatchNumber 43}
  {:RoundNumber 3,
   :Group "Group F",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Jamaica",
   :DateUtc "2023-08-02 10:00:00Z",
   :AwayTeam "Brazil",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 44}
  {:RoundNumber 3,
   :Group "Group H",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Korea Republic",
   :DateUtc "2023-08-03 10:00:00Z",
   :AwayTeam "Germany",
   :Location "Brisbane Stadium",
   :MatchNumber 47}
  {:RoundNumber 3,
   :Group "Group H",
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "Morocco",
   :DateUtc "2023-08-03 10:00:00Z",
   :AwayTeam "Colombia",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 48}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1A",
   :DateUtc "2023-08-05 05:00:00Z",
   :AwayTeam "2C",
   :Location "Eden Park",
   :MatchNumber 49}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1C",
   :DateUtc "2023-08-05 08:00:00Z",
   :AwayTeam "2A",
   :Location "Wellington Regional Stadium",
   :MatchNumber 50}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1E",
   :DateUtc "2023-08-06 02:00:00Z",
   :AwayTeam "2G",
   :Location "Sydney Football Stadium",
   :MatchNumber 51}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1G",
   :DateUtc "2023-08-06 09:00:00Z",
   :AwayTeam "2E",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 52}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1D",
   :DateUtc "2023-08-07 07:30:00Z",
   :AwayTeam "2B",
   :Location "Brisbane Stadium",
   :MatchNumber 54}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1B",
   :DateUtc "2023-08-07 10:30:00Z",
   :AwayTeam "2D",
   :Location "Stadium Australia",
   :MatchNumber 53}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1H",
   :DateUtc "2023-08-08 08:00:00Z",
   :AwayTeam "2F",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 56}
  {:RoundNumber 4,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "1F",
   :DateUtc "2023-08-08 11:00:00Z",
   :AwayTeam "2H",
   :Location "Hindmarsh Stadium",
   :MatchNumber 55}
  {:RoundNumber 5,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-11 01:00:00Z",
   :AwayTeam "To be announced",
   :Location "Wellington Regional Stadium",
   :MatchNumber 57}
  {:RoundNumber 5,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-11 07:30:00Z",
   :AwayTeam "To be announced",
   :Location "Eden Park",
   :MatchNumber 58}
  {:RoundNumber 5,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-12 07:00:00Z",
   :AwayTeam "To be announced",
   :Location "Brisbane Stadium",
   :MatchNumber 59}
  {:RoundNumber 5,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-12 10:30:00Z",
   :AwayTeam "To be announced",
   :Location "Stadium Australia",
   :MatchNumber 60}
  {:RoundNumber 6,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-15 08:00:00Z",
   :AwayTeam "To be announced",
   :Location "Eden Park",
   :MatchNumber 61}
  {:RoundNumber 6,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-16 10:00:00Z",
   :AwayTeam "To be announced",
   :Location "Stadium Australia",
   :MatchNumber 62}
  {:RoundNumber 7,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-19 08:00:00Z",
   :AwayTeam "To be announced",
   :Location "Brisbane Stadium",
   :MatchNumber 63}
  {:RoundNumber 7,
   :Group nil,
   :HomeTeamScore nil,
   :AwayTeamScore nil,
   :HomeTeam "To be announced",
   :DateUtc "2023-08-20 10:00:00Z",
   :AwayTeam "To be announced",
   :Location "Stadium Australia",
   :MatchNumber 64}],
 :results
 ({:RoundNumber 1,
   :Group "Group A",
   :game 1,
   :HomeTeamScore nil,
   :teams [:nzl :nor],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:nzl {}, :nor {}},
   :score {:nzl "", :nor ""},
   :comment "",
   :HomeTeam "New Zealand",
   :DateUtc "2023-07-20 07:00:00Z",
   :scoreboard {:nzl "", :nor ""},
   :AwayTeam "Norway",
   :Location "Eden Park",
   :MatchNumber 1}
  {:RoundNumber 1,
   :Group "Group B",
   :game 2,
   :HomeTeamScore nil,
   :teams [:aus :irl],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:aus {}, :irl {}},
   :score {:aus "", :irl ""},
   :comment "",
   :HomeTeam "Australia",
   :DateUtc "2023-07-20 10:00:00Z",
   :scoreboard {:aus "", :irl ""},
   :AwayTeam "Republic of Ireland",
   :Location "Sydney Football Stadium",
   :MatchNumber 2}
  {:RoundNumber 1,
   :Group "Group B",
   :game 4,
   :HomeTeamScore nil,
   :teams [:nig :can],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:nig {}, :can {}},
   :score {:nig "", :can ""},
   :comment "",
   :HomeTeam "Nigeria",
   :DateUtc "2023-07-21 02:30:00Z",
   :scoreboard {:nig "", :can ""},
   :AwayTeam "Canada",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 4}
  {:RoundNumber 1,
   :Group "Group A",
   :game 3,
   :HomeTeamScore nil,
   :teams [:phi :sui],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:phi {}, :sui {}},
   :score {:phi "", :sui ""},
   :comment "",
   :HomeTeam "Philippines",
   :DateUtc "2023-07-21 05:00:00Z",
   :scoreboard {:phi "", :sui ""},
   :AwayTeam "Switzerland",
   :Location "Dunedin Stadium",
   :MatchNumber 3}
  {:RoundNumber 1,
   :Group "Group C",
   :game 5,
   :HomeTeamScore nil,
   :teams [:esp :cos],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:esp {}, :cos {}},
   :score {:esp "", :cos ""},
   :comment "",
   :HomeTeam "Spain",
   :DateUtc "2023-07-21 07:30:00Z",
   :scoreboard {:esp "", :cos ""},
   :AwayTeam "Costa Rica",
   :Location "Wellington Regional Stadium",
   :MatchNumber 5}
  {:RoundNumber 1,
   :Group "Group E",
   :game 9,
   :HomeTeamScore nil,
   :teams [:usa :vie],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:usa {}, :vie {}},
   :score {:usa "", :vie ""},
   :comment "",
   :HomeTeam "USA",
   :DateUtc "2023-07-22 01:00:00Z",
   :scoreboard {:usa "", :vie ""},
   :AwayTeam "Vietnam",
   :Location "Eden Park",
   :MatchNumber 9}
  {:RoundNumber 1,
   :Group "Group C",
   :game 6,
   :HomeTeamScore nil,
   :teams [:zam :jpn],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:zam {}, :jpn {}},
   :score {:zam "", :jpn ""},
   :comment "",
   :HomeTeam "Zambia",
   :DateUtc "2023-07-22 07:00:00Z",
   :scoreboard {:zam "", :jpn ""},
   :AwayTeam "Japan",
   :Location "Waikato Stadium",
   :MatchNumber 6}
  {:RoundNumber 1,
   :Group "Group D",
   :game 7,
   :HomeTeamScore nil,
   :teams [:eng :group-b-play-off-winner],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:eng {}, :group-b-play-off-winner {}},
   :score {:eng "", :group-b-play-off-winner ""},
   :comment "",
   :HomeTeam "England",
   :DateUtc "2023-07-22 09:30:00Z",
   :scoreboard {:eng "", :group-b-play-off-winner ""},
   :AwayTeam "Group B Play-Off Winner",
   :Location "Brisbane Stadium",
   :MatchNumber 7}
  {:RoundNumber 1,
   :Group "Group D",
   :game 8,
   :HomeTeamScore nil,
   :teams [:den :chn],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:den {}, :chn {}},
   :score {:den "", :chn ""},
   :comment "",
   :HomeTeam "Denmark",
   :DateUtc "2023-07-22 12:00:00Z",
   :scoreboard {:den "", :chn ""},
   :AwayTeam "China PR",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 8}
  {:RoundNumber 1,
   :Group "Group G",
   :game 12,
   :HomeTeamScore nil,
   :teams [:swe :rsa],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:swe {}, :rsa {}},
   :score {:swe "", :rsa ""},
   :comment "",
   :HomeTeam "Sweden",
   :DateUtc "2023-07-23 05:00:00Z",
   :scoreboard {:swe "", :rsa ""},
   :AwayTeam "South Africa",
   :Location "Wellington Regional Stadium",
   :MatchNumber 12}
  {:RoundNumber 1,
   :Group "Group E",
   :game 10,
   :HomeTeamScore nil,
   :teams [:ned :group-a-play-off-winner],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:ned {}, :group-a-play-off-winner {}},
   :score {:ned "", :group-a-play-off-winner ""},
   :comment "",
   :HomeTeam "Netherlands",
   :DateUtc "2023-07-23 07:30:00Z",
   :scoreboard {:ned "", :group-a-play-off-winner ""},
   :AwayTeam "Group A Play-Off Winner",
   :Location "Dunedin Stadium",
   :MatchNumber 10}
  {:RoundNumber 1,
   :Group "Group F",
   :game 11,
   :HomeTeamScore nil,
   :teams [:fra :jam],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:fra {}, :jam {}},
   :score {:fra "", :jam ""},
   :comment "",
   :HomeTeam "France",
   :DateUtc "2023-07-23 10:00:00Z",
   :scoreboard {:fra "", :jam ""},
   :AwayTeam "Jamaica",
   :Location "Sydney Football Stadium",
   :MatchNumber 11}
  {:RoundNumber 1,
   :Group "Group G",
   :game 14,
   :HomeTeamScore nil,
   :teams [:ita :arg],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:ita {}, :arg {}},
   :score {:ita "", :arg ""},
   :comment "",
   :HomeTeam "Italy",
   :DateUtc "2023-07-24 06:00:00Z",
   :scoreboard {:ita "", :arg ""},
   :AwayTeam "Argentina",
   :Location "Eden Park",
   :MatchNumber 14}
  {:RoundNumber 1,
   :Group "Group H",
   :game 15,
   :HomeTeamScore nil,
   :teams [:ger :mar],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:ger {}, :mar {}},
   :score {:ger "", :mar ""},
   :comment "",
   :HomeTeam "Germany",
   :DateUtc "2023-07-24 08:30:00Z",
   :scoreboard {:ger "", :mar ""},
   :AwayTeam "Morocco",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 15}
  {:RoundNumber 1,
   :Group "Group F",
   :game 13,
   :HomeTeamScore nil,
   :teams [:bra :group-c-play-off-winner],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:bra {}, :group-c-play-off-winner {}},
   :score {:bra "", :group-c-play-off-winner ""},
   :comment "",
   :HomeTeam "Brazil",
   :DateUtc "2023-07-24 11:00:00Z",
   :scoreboard {:bra "", :group-c-play-off-winner ""},
   :AwayTeam "Group C Play-Off Winner",
   :Location "Hindmarsh Stadium",
   :MatchNumber 13}
  {:RoundNumber 1,
   :Group "Group H",
   :game 16,
   :HomeTeamScore nil,
   :teams [:col :kor],
   :round 1,
   :summary "",
   :AwayTeamScore nil,
   :result {:col {}, :kor {}},
   :score {:col "", :kor ""},
   :comment "",
   :HomeTeam "Colombia",
   :DateUtc "2023-07-25 02:00:00Z",
   :scoreboard {:col "", :kor ""},
   :AwayTeam "Korea Republic",
   :Location "Sydney Football Stadium",
   :MatchNumber 16}
  {:RoundNumber 2,
   :Group "Group A",
   :game 17,
   :HomeTeamScore nil,
   :teams [:nzl :phi],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:nzl {}, :phi {}},
   :score {:nzl "", :phi ""},
   :comment "",
   :HomeTeam "New Zealand",
   :DateUtc "2023-07-25 05:30:00Z",
   :scoreboard {:nzl "", :phi ""},
   :AwayTeam "Philippines",
   :Location "Wellington Regional Stadium",
   :MatchNumber 17}
  {:RoundNumber 2,
   :Group "Group A",
   :game 18,
   :HomeTeamScore nil,
   :teams [:sui :nor],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:sui {}, :nor {}},
   :score {:sui "", :nor ""},
   :comment "",
   :HomeTeam "Switzerland",
   :DateUtc "2023-07-25 08:00:00Z",
   :scoreboard {:sui "", :nor ""},
   :AwayTeam "Norway",
   :Location "Waikato Stadium",
   :MatchNumber 18}
  {:RoundNumber 2,
   :Group "Group C",
   :game 21,
   :HomeTeamScore nil,
   :teams [:jpn :cos],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:jpn {}, :cos {}},
   :score {:jpn "", :cos ""},
   :comment "",
   :HomeTeam "Japan",
   :DateUtc "2023-07-26 05:00:00Z",
   :scoreboard {:jpn "", :cos ""},
   :AwayTeam "Costa Rica",
   :Location "Dunedin Stadium",
   :MatchNumber 21}
  {:RoundNumber 2,
   :Group "Group C",
   :game 20,
   :HomeTeamScore nil,
   :teams [:esp :zam],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:esp {}, :zam {}},
   :score {:esp "", :zam ""},
   :comment "",
   :HomeTeam "Spain",
   :DateUtc "2023-07-26 07:30:00Z",
   :scoreboard {:esp "", :zam ""},
   :AwayTeam "Zambia",
   :Location "Eden Park",
   :MatchNumber 20}
  {:RoundNumber 2,
   :Group "Group B",
   :game 19,
   :HomeTeamScore nil,
   :teams [:can :irl],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:can {}, :irl {}},
   :score {:can "", :irl ""},
   :comment "",
   :HomeTeam "Canada",
   :DateUtc "2023-07-26 12:00:00Z",
   :scoreboard {:can "", :irl ""},
   :AwayTeam "Republic of Ireland",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 19}
  {:RoundNumber 2,
   :Group "Group E",
   :game 23,
   :HomeTeamScore nil,
   :teams [:usa :ned],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:usa {}, :ned {}},
   :score {:usa "", :ned ""},
   :comment "",
   :HomeTeam "USA",
   :DateUtc "2023-07-27 01:00:00Z",
   :scoreboard {:usa "", :ned ""},
   :AwayTeam "Netherlands",
   :Location "Wellington Regional Stadium",
   :MatchNumber 23}
  {:RoundNumber 2,
   :Group "Group E",
   :game 24,
   :HomeTeamScore nil,
   :teams [:group-a-play-off-winner :vie],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:group-a-play-off-winner {}, :vie {}},
   :score {:group-a-play-off-winner "", :vie ""},
   :comment "",
   :HomeTeam "Group A Play-Off Winner",
   :DateUtc "2023-07-27 07:30:00Z",
   :scoreboard {:group-a-play-off-winner "", :vie ""},
   :AwayTeam "Vietnam",
   :Location "Waikato Stadium",
   :MatchNumber 24}
  {:RoundNumber 2,
   :Group "Group B",
   :game 22,
   :HomeTeamScore nil,
   :teams [:aus :nig],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:aus {}, :nig {}},
   :score {:aus "", :nig ""},
   :comment "",
   :HomeTeam "Australia",
   :DateUtc "2023-07-27 10:00:00Z",
   :scoreboard {:aus "", :nig ""},
   :AwayTeam "Nigeria",
   :Location "Brisbane Stadium",
   :MatchNumber 22}
  {:RoundNumber 2,
   :Group "Group G",
   :game 27,
   :HomeTeamScore nil,
   :teams [:arg :rsa],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:arg {}, :rsa {}},
   :score {:arg "", :rsa ""},
   :comment "",
   :HomeTeam "Argentina",
   :DateUtc "2023-07-28 00:00:00Z",
   :scoreboard {:arg "", :rsa ""},
   :AwayTeam "South Africa",
   :Location "Dunedin Stadium",
   :MatchNumber 27}
  {:RoundNumber 2,
   :Group "Group D",
   :game 25,
   :HomeTeamScore nil,
   :teams [:eng :den],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:eng {}, :den {}},
   :score {:eng "", :den ""},
   :comment "",
   :HomeTeam "England",
   :DateUtc "2023-07-28 08:30:00Z",
   :scoreboard {:eng "", :den ""},
   :AwayTeam "Denmark",
   :Location "Sydney Football Stadium",
   :MatchNumber 25}
  {:RoundNumber 2,
   :Group "Group D",
   :game 26,
   :HomeTeamScore nil,
   :teams [:chn :group-b-play-off-winner],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:chn {}, :group-b-play-off-winner {}},
   :score {:chn "", :group-b-play-off-winner ""},
   :comment "",
   :HomeTeam "China PR",
   :DateUtc "2023-07-28 11:00:00Z",
   :scoreboard {:chn "", :group-b-play-off-winner ""},
   :AwayTeam "Group B Play-Off Winner",
   :Location "Hindmarsh Stadium",
   :MatchNumber 26}
  {:RoundNumber 2,
   :Group "Group G",
   :game 30,
   :HomeTeamScore nil,
   :teams [:swe :ita],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:swe {}, :ita {}},
   :score {:swe "", :ita ""},
   :comment "",
   :HomeTeam "Sweden",
   :DateUtc "2023-07-29 07:30:00Z",
   :scoreboard {:swe "", :ita ""},
   :AwayTeam "Italy",
   :Location "Wellington Regional Stadium",
   :MatchNumber 30}
  {:RoundNumber 2,
   :Group "Group F",
   :game 28,
   :HomeTeamScore nil,
   :teams [:fra :bra],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:fra {}, :bra {}},
   :score {:fra "", :bra ""},
   :comment "",
   :HomeTeam "France",
   :DateUtc "2023-07-29 10:00:00Z",
   :scoreboard {:fra "", :bra ""},
   :AwayTeam "Brazil",
   :Location "Brisbane Stadium",
   :MatchNumber 28}
  {:RoundNumber 2,
   :Group "Group F",
   :game 29,
   :HomeTeamScore nil,
   :teams [:group-c-play-off-winner :jam],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:group-c-play-off-winner {}, :jam {}},
   :score {:group-c-play-off-winner "", :jam ""},
   :comment "",
   :HomeTeam "Group C Play-Off Winner",
   :DateUtc "2023-07-29 12:30:00Z",
   :scoreboard {:group-c-play-off-winner "", :jam ""},
   :AwayTeam "Jamaica",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 29}
  {:RoundNumber 2,
   :Group "Group H",
   :game 32,
   :HomeTeamScore nil,
   :teams [:kor :mar],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:kor {}, :mar {}},
   :score {:kor "", :mar ""},
   :comment "",
   :HomeTeam "Korea Republic",
   :DateUtc "2023-07-30 04:30:00Z",
   :scoreboard {:kor "", :mar ""},
   :AwayTeam "Morocco",
   :Location "Hindmarsh Stadium",
   :MatchNumber 32}
  {:RoundNumber 2,
   :Group "Group H",
   :game 31,
   :HomeTeamScore nil,
   :teams [:ger :col],
   :round 2,
   :summary "",
   :AwayTeamScore nil,
   :result {:ger {}, :col {}},
   :score {:ger "", :col ""},
   :comment "",
   :HomeTeam "Germany",
   :DateUtc "2023-07-30 09:30:00Z",
   :scoreboard {:ger "", :col ""},
   :AwayTeam "Colombia",
   :Location "Sydney Football Stadium",
   :MatchNumber 31}
  {:RoundNumber 3,
   :Group "Group A",
   :game 33,
   :HomeTeamScore nil,
   :teams [:sui :nzl],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:sui {}, :nzl {}},
   :score {:sui "", :nzl ""},
   :comment "",
   :HomeTeam "Switzerland",
   :DateUtc "2023-07-30 07:00:00Z",
   :scoreboard {:sui "", :nzl ""},
   :AwayTeam "New Zealand",
   :Location "Dunedin Stadium",
   :MatchNumber 33}
  {:RoundNumber 3,
   :Group "Group A",
   :game 34,
   :HomeTeamScore nil,
   :teams [:nor :phi],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:nor {}, :phi {}},
   :score {:nor "", :phi ""},
   :comment "",
   :HomeTeam "Norway",
   :DateUtc "2023-07-30 07:00:00Z",
   :scoreboard {:nor "", :phi ""},
   :AwayTeam "Philippines",
   :Location "Eden Park",
   :MatchNumber 34}
  {:RoundNumber 3,
   :Group "Group C",
   :game 37,
   :HomeTeamScore nil,
   :teams [:jpn :esp],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:jpn {}, :esp {}},
   :score {:jpn "", :esp ""},
   :comment "",
   :HomeTeam "Japan",
   :DateUtc "2023-07-31 07:00:00Z",
   :scoreboard {:jpn "", :esp ""},
   :AwayTeam "Spain",
   :Location "Wellington Regional Stadium",
   :MatchNumber 37}
  {:RoundNumber 3,
   :Group "Group C",
   :game 38,
   :HomeTeamScore nil,
   :teams [:cos :zam],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:cos {}, :zam {}},
   :score {:cos "", :zam ""},
   :comment "",
   :HomeTeam "Costa Rica",
   :DateUtc "2023-07-31 07:00:00Z",
   :scoreboard {:cos "", :zam ""},
   :AwayTeam "Zambia",
   :Location "Waikato Stadium",
   :MatchNumber 38}
  {:RoundNumber 3,
   :Group "Group B",
   :game 35,
   :HomeTeamScore nil,
   :teams [:can :aus],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:can {}, :aus {}},
   :score {:can "", :aus ""},
   :comment "",
   :HomeTeam "Canada",
   :DateUtc "2023-07-31 10:00:00Z",
   :scoreboard {:can "", :aus ""},
   :AwayTeam "Australia",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 35}
  {:RoundNumber 3,
   :Group "Group B",
   :game 36,
   :HomeTeamScore nil,
   :teams [:irl :nig],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:irl {}, :nig {}},
   :score {:irl "", :nig ""},
   :comment "",
   :HomeTeam "Republic of Ireland",
   :DateUtc "2023-07-31 10:00:00Z",
   :scoreboard {:irl "", :nig ""},
   :AwayTeam "Nigeria",
   :Location "Brisbane Stadium",
   :MatchNumber 36}
  {:RoundNumber 3,
   :Group "Group E",
   :game 41,
   :HomeTeamScore nil,
   :teams [:group-a-play-off-winner :usa],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:group-a-play-off-winner {}, :usa {}},
   :score {:group-a-play-off-winner "", :usa ""},
   :comment "",
   :HomeTeam "Group A Play-Off Winner",
   :DateUtc "2023-08-01 07:00:00Z",
   :scoreboard {:group-a-play-off-winner "", :usa ""},
   :AwayTeam "USA",
   :Location "Eden Park",
   :MatchNumber 41}
  {:RoundNumber 3,
   :Group "Group E",
   :game 42,
   :HomeTeamScore nil,
   :teams [:vie :ned],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:vie {}, :ned {}},
   :score {:vie "", :ned ""},
   :comment "",
   :HomeTeam "Vietnam",
   :DateUtc "2023-08-01 07:00:00Z",
   :scoreboard {:vie "", :ned ""},
   :AwayTeam "Netherlands",
   :Location "Dunedin Stadium",
   :MatchNumber 42}
  {:RoundNumber 3,
   :Group "Group D",
   :game 39,
   :HomeTeamScore nil,
   :teams [:chn :eng],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:chn {}, :eng {}},
   :score {:chn "", :eng ""},
   :comment "",
   :HomeTeam "China PR",
   :DateUtc "2023-08-01 11:00:00Z",
   :scoreboard {:chn "", :eng ""},
   :AwayTeam "England",
   :Location "Hindmarsh Stadium",
   :MatchNumber 39}
  {:RoundNumber 3,
   :Group "Group D",
   :game 40,
   :HomeTeamScore nil,
   :teams [:group-b-play-off-winner :den],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:group-b-play-off-winner {}, :den {}},
   :score {:group-b-play-off-winner "", :den ""},
   :comment "",
   :HomeTeam "Group B Play-Off Winner",
   :DateUtc "2023-08-01 11:00:00Z",
   :scoreboard {:group-b-play-off-winner "", :den ""},
   :AwayTeam "Denmark",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 40}
  {:RoundNumber 3,
   :Group "Group G",
   :game 45,
   :HomeTeamScore nil,
   :teams [:arg :swe],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:arg {}, :swe {}},
   :score {:arg "", :swe ""},
   :comment "",
   :HomeTeam "Argentina",
   :DateUtc "2023-08-02 07:00:00Z",
   :scoreboard {:arg "", :swe ""},
   :AwayTeam "Sweden",
   :Location "Waikato Stadium",
   :MatchNumber 45}
  {:RoundNumber 3,
   :Group "Group G",
   :game 46,
   :HomeTeamScore nil,
   :teams [:rsa :ita],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:rsa {}, :ita {}},
   :score {:rsa "", :ita ""},
   :comment "",
   :HomeTeam "South Africa",
   :DateUtc "2023-08-02 07:00:00Z",
   :scoreboard {:rsa "", :ita ""},
   :AwayTeam "Italy",
   :Location "Wellington Regional Stadium",
   :MatchNumber 46}
  {:RoundNumber 3,
   :Group "Group F",
   :game 43,
   :HomeTeamScore nil,
   :teams [:group-c-play-off-winner :fra],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:group-c-play-off-winner {}, :fra {}},
   :score {:group-c-play-off-winner "", :fra ""},
   :comment "",
   :HomeTeam "Group C Play-Off Winner",
   :DateUtc "2023-08-02 10:00:00Z",
   :scoreboard {:group-c-play-off-winner "", :fra ""},
   :AwayTeam "France",
   :Location "Sydney Football Stadium",
   :MatchNumber 43}
  {:RoundNumber 3,
   :Group "Group F",
   :game 44,
   :HomeTeamScore nil,
   :teams [:jam :bra],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:jam {}, :bra {}},
   :score {:jam "", :bra ""},
   :comment "",
   :HomeTeam "Jamaica",
   :DateUtc "2023-08-02 10:00:00Z",
   :scoreboard {:jam "", :bra ""},
   :AwayTeam "Brazil",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 44}
  {:RoundNumber 3,
   :Group "Group H",
   :game 47,
   :HomeTeamScore nil,
   :teams [:kor :ger],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:kor {}, :ger {}},
   :score {:kor "", :ger ""},
   :comment "",
   :HomeTeam "Korea Republic",
   :DateUtc "2023-08-03 10:00:00Z",
   :scoreboard {:kor "", :ger ""},
   :AwayTeam "Germany",
   :Location "Brisbane Stadium",
   :MatchNumber 47}
  {:RoundNumber 3,
   :Group "Group H",
   :game 48,
   :HomeTeamScore nil,
   :teams [:mar :col],
   :round 3,
   :summary "",
   :AwayTeamScore nil,
   :result {:mar {}, :col {}},
   :score {:mar "", :col ""},
   :comment "",
   :HomeTeam "Morocco",
   :DateUtc "2023-08-03 10:00:00Z",
   :scoreboard {:mar "", :col ""},
   :AwayTeam "Colombia",
   :Location "Perth Rectangular Stadium",
   :MatchNumber 48}
  {:RoundNumber 4,
   :Group nil,
   :game 49,
   :HomeTeamScore nil,
   :teams [:1a :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1a {}, :away {}},
   :score {:1a "", :away ""},
   :comment "",
   :HomeTeam "1A",
   :DateUtc "2023-08-05 05:00:00Z",
   :scoreboard {:1a "", :away ""},
   :AwayTeam "2C",
   :Location "Eden Park",
   :MatchNumber 49}
  {:RoundNumber 4,
   :Group nil,
   :game 50,
   :HomeTeamScore nil,
   :teams [:1c :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1c {}, :away {}},
   :score {:1c "", :away ""},
   :comment "",
   :HomeTeam "1C",
   :DateUtc "2023-08-05 08:00:00Z",
   :scoreboard {:1c "", :away ""},
   :AwayTeam "2A",
   :Location "Wellington Regional Stadium",
   :MatchNumber 50}
  {:RoundNumber 4,
   :Group nil,
   :game 51,
   :HomeTeamScore nil,
   :teams [:1e :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1e {}, :away {}},
   :score {:1e "", :away ""},
   :comment "",
   :HomeTeam "1E",
   :DateUtc "2023-08-06 02:00:00Z",
   :scoreboard {:1e "", :away ""},
   :AwayTeam "2G",
   :Location "Sydney Football Stadium",
   :MatchNumber 51}
  {:RoundNumber 4,
   :Group nil,
   :game 52,
   :HomeTeamScore nil,
   :teams [:1g :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1g {}, :away {}},
   :score {:1g "", :away ""},
   :comment "",
   :HomeTeam "1G",
   :DateUtc "2023-08-06 09:00:00Z",
   :scoreboard {:1g "", :away ""},
   :AwayTeam "2E",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 52}
  {:RoundNumber 4,
   :Group nil,
   :game 54,
   :HomeTeamScore nil,
   :teams [:1d :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1d {}, :away {}},
   :score {:1d "", :away ""},
   :comment "",
   :HomeTeam "1D",
   :DateUtc "2023-08-07 07:30:00Z",
   :scoreboard {:1d "", :away ""},
   :AwayTeam "2B",
   :Location "Brisbane Stadium",
   :MatchNumber 54}
  {:RoundNumber 4,
   :Group nil,
   :game 53,
   :HomeTeamScore nil,
   :teams [:1b :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1b {}, :away {}},
   :score {:1b "", :away ""},
   :comment "",
   :HomeTeam "1B",
   :DateUtc "2023-08-07 10:30:00Z",
   :scoreboard {:1b "", :away ""},
   :AwayTeam "2D",
   :Location "Stadium Australia",
   :MatchNumber 53}
  {:RoundNumber 4,
   :Group nil,
   :game 56,
   :HomeTeamScore nil,
   :teams [:1h :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1h {}, :away {}},
   :score {:1h "", :away ""},
   :comment "",
   :HomeTeam "1H",
   :DateUtc "2023-08-08 08:00:00Z",
   :scoreboard {:1h "", :away ""},
   :AwayTeam "2F",
   :Location "Melbourne Rectangular Stadium",
   :MatchNumber 56}
  {:RoundNumber 4,
   :Group nil,
   :game 55,
   :HomeTeamScore nil,
   :teams [:1f :away],
   :round 4,
   :summary "",
   :AwayTeamScore nil,
   :result {:1f {}, :away {}},
   :score {:1f "", :away ""},
   :comment "",
   :HomeTeam "1F",
   :DateUtc "2023-08-08 11:00:00Z",
   :scoreboard {:1f "", :away ""},
   :AwayTeam "2H",
   :Location "Hindmarsh Stadium",
   :MatchNumber 55}
  {:RoundNumber 5,
   :Group nil,
   :game 57,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 5,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-11 01:00:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Wellington Regional Stadium",
   :MatchNumber 57}
  {:RoundNumber 5,
   :Group nil,
   :game 58,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 5,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-11 07:30:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Eden Park",
   :MatchNumber 58}
  {:RoundNumber 5,
   :Group nil,
   :game 59,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 5,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-12 07:00:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Brisbane Stadium",
   :MatchNumber 59}
  {:RoundNumber 5,
   :Group nil,
   :game 60,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 5,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-12 10:30:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Stadium Australia",
   :MatchNumber 60}
  {:RoundNumber 6,
   :Group nil,
   :game 61,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 6,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-15 08:00:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Eden Park",
   :MatchNumber 61}
  {:RoundNumber 6,
   :Group nil,
   :game 62,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 6,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-16 10:00:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Stadium Australia",
   :MatchNumber 62}
  {:RoundNumber 7,
   :Group nil,
   :game 63,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 7,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-19 08:00:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Brisbane Stadium",
   :MatchNumber 63}
  {:RoundNumber 7,
   :Group nil,
   :game 64,
   :HomeTeamScore nil,
   :teams [:home :away],
   :round 7,
   :summary "",
   :AwayTeamScore nil,
   :result {:home {}, :away {}},
   :score {:home "", :away ""},
   :comment "",
   :HomeTeam "To be announced",
   :DateUtc "2023-08-20 10:00:00Z",
   :scoreboard {:home "", :away ""},
   :AwayTeam "To be announced",
   :Location "Stadium Australia",
   :MatchNumber 64}),
 :teams
 {"Australia" {:id "Australia", :name "Australia"},
  "1A" {:id "1A", :name "1A"},
  "Argentina" {:id "Argentina", :name "Argentina"},
  "New Zealand" {:id "New Zealand", :name "New Zealand"},
  "Republic of Ireland"
  {:id "Republic of Ireland", :name "Republic of Ireland"},
  "Brazil" {:id "Brazil", :name "Brazil"},
  "Colombia" {:id "Colombia", :name "Colombia"},
  "Sweden" {:id "Sweden", :name "Sweden"},
  "Canada" {:id "Canada", :name "Canada"},
  "South Africa" {:id "South Africa", :name "South Africa"},
  "Jamaica" {:id "Jamaica", :name "Jamaica"},
  "1C" {:id "1C", :name "1C"},
  "Group B Play-Off Winner"
  {:id "Group B Play-Off Winner", :name "Group B Play-Off Winner"},
  "Vietnam" {:id "Vietnam", :name "Vietnam"},
  "To be announced" {:id "To be announced", :name "To be announced"},
  "Japan" {:id "Japan", :name "Japan"},
  "Nigeria" {:id "Nigeria", :name "Nigeria"},
  "1F" {:id "1F", :name "1F"},
  "1B" {:id "1B", :name "1B"},
  "Philippines" {:id "Philippines", :name "Philippines"},
  "Italy" {:id "Italy", :name "Italy"},
  "France" {:id "France", :name "France"},
  "Korea Republic" {:id "Korea Republic", :name "Korea Republic"},
  "Group C Play-Off Winner"
  {:id "Group C Play-Off Winner", :name "Group C Play-Off Winner"},
  "Group A Play-Off Winner"
  {:id "Group A Play-Off Winner", :name "Group A Play-Off Winner"},
  "Norway" {:id "Norway", :name "Norway"},
  "1D" {:id "1D", :name "1D"},
  "Denmark" {:id "Denmark", :name "Denmark"},
  "Switzerland" {:id "Switzerland", :name "Switzerland"},
  "Netherlands" {:id "Netherlands", :name "Netherlands"},
  "Spain" {:id "Spain", :name "Spain"},
  "Germany" {:id "Germany", :name "Germany"},
  "1E" {:id "1E", :name "1E"},
  "USA" {:id "USA", :name "USA"},
  "1G" {:id "1G", :name "1G"},
  "Costa Rica" {:id "Costa Rica", :name "Costa Rica"},
  "1H" {:id "1H", :name "1H"},
  "Morocco" {:id "Morocco", :name "Morocco"},
  "England" {:id "England", :name "England"},
  "Zambia" {:id "Zambia", :name "Zambia"},
  "China PR" {:id "China PR", :name "China PR"}},
 :venues
 {"Stadium Australia"
  {:id "Stadium Australia", :name "Stadium Australia"},
  "Dunedin Stadium" {:id "Dunedin Stadium", :name "Dunedin Stadium"},
  "Eden Park" {:id "Eden Park", :name "Eden Park"},
  "Perth Rectangular Stadium"
  {:id "Perth Rectangular Stadium", :name "Perth Rectangular Stadium"},
  "Brisbane Stadium"
  {:id "Brisbane Stadium", :name "Brisbane Stadium"},
  "Sydney Football Stadium"
  {:id "Sydney Football Stadium", :name "Sydney Football Stadium"},
  "Waikato Stadium" {:id "Waikato Stadium", :name "Waikato Stadium"},
  "Melbourne Rectangular Stadium"
  {:id "Melbourne Rectangular Stadium",
   :name "Melbourne Rectangular Stadium"},
  "Hindmarsh Stadium"
  {:id "Hindmarsh Stadium", :name "Hindmarsh Stadium"},
  "Wellington Regional Stadium"
  {:id "Wellington Regional Stadium",
   :name "Wellington Regional Stadium"}}}